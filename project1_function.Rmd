...
plots <- function(df, genes_list, cont_cov, cat_cov) {
  ...
  df_filtered_long <- df_filtered %>% tidyr::pivot_longer(cols = genes_list, names_to = 'Gene', values_to = 'Expression') %>%
  ...
  annotate(geom = 'text', x = x_min_box + 15, y = 12, label = paste('N:', n,' samples', 
    ...
    scatter <- scatter_filtered %>% ggplot(aes(x = .data[[cont_cov]], y = Expression, color = crp.level)) +
      ...
      scale_color_manual(labels = c('Normal (<3mg/L)', 'Moderate (3-10mg/L)', 'High (10 -100mg/L)', 'Severe (>100mg/L)'), values =
    ...
    #all_plots <- ggpubr::ggarrange(plotlist = list(box, scatter, hist), ncol = 2, nrow = 2)
    #print(all_plots)
  } 
  }
...
plots(all_data, c('ABI1', 'AAMP', 'ABHD17A'), c('crp.mg.l.'), c('disease_status', 'mechanical_ventilation'))
