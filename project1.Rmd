...
ggplot(all_data_long[(126*90):(126*100),], aes(x = Gene, y = count, color = disease_status)) +
  geom_boxplot() +
  theme(
...
ggplot(all_data, aes(ABHD17A)) +
  ...
  labs(title = 'Distribution of ABHD17A count across samples', x = 'ABHD17A Count', y = 'Frequency', size = 20) +
...
ggplot(all_data %>% filter(crp.mg.l. != ' unknown') %>% dplyr::arrange(crp.mg.l.), aes(x = crp.mg.l. , y =  ABHD17A , color = disease_status)) +
  ...
  labs(title = 'CRP levels vs ABHD17A count', x = 'CRP Level (mg/L)', y = 'ABHD17A count') +
...
dplyr::filter(curated_genes_long, Gene == 'ABHD17A') %>% 
  ...
  labs(title = 'Comparing ABHD17A levels across disease status and mechanical ventilation', x = 'Disease Status', y = 'ABHD17A count') +
